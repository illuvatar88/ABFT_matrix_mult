...
void matrix_transpose(int *A, int *C, int num_threads);
...
int main (int argc, char *argv[]) {
    ...
    int *A = new int[(SIZE_R + 1) * (SIZE_C + 1)];    //input matrix
    int *C = new int[(SIZE_R + 1) * (SIZE_C + 1)];    //output matrix
    int i, j, k;
...
void matrix_transpose(int *A, int *C, int num_threads) {
    int i, j, k;
    #pragma omp parallel for shared(A, C, num_threads) private(i, j) schedule(static, (SIZE_R + 1) / num_threads)
    for (i = 0 ; i <= SIZE_R ; i++) {
        for (j = 0 ; j <= SIZE_C ; j++) {
            C[(SIZE_R + 1) * j + i] = A[i * (SIZE_C + 1) + j];
        }
    }
}
